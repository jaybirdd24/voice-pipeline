...
- **Training:** fine-tune from an existing Piper medium checkpoint using the established training scripts and gradient-accumulation setup. English phonemizes with `en-GB-x-rp` — reviewed against the en-AU target 2026-07: espeak-ng has no en-AU voice, so the closest British RP voice is retained (decision recorded in `docs/training-notes.md`).
