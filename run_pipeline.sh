...
# Usage: ./run_pipeline.sh --lang en [--count 500] [--max-epochs 6440]
...
DATASETS_DIR="$(python3 -c "import yaml; cfg=yaml.safe_load(open('$CONFIG')); print(cfg['datasets_dir'])")"
DATASET_DIR="${DATASETS_DIR}/${LANG}_qwen3_synth"
EXPORT_DIR="$SCRIPT_DIR/export/${LANG}_pi_bundle"

echo "================================================================"
echo " Voice Pipeline — $LANG"
...
if $SKIP_GENERATE; then
    ...
    echo "[Stage 1] Generating synthetic dataset ($COUNT clips, lang=$LANG)..."
...
if $SKIP_TRAIN; then
    ...
    echo "[Stage 2] Training Piper (lang=$LANG, max_epochs=$MAX_EPOCHS)..."
    echo "          Watch TensorBoard: tensorboard --logdir /home/jay/p4p/piper_training/training_output_${LANG}_qwen3_synth"
    MAX_EPOCHS="$MAX_EPOCHS" bash "$SCRIPT_DIR/train_piper.sh" --lang "$LANG"
    echo "[Stage 2] Done."
fi
...
if [ -d "$EXPORT_DIR" ] && ls "$EXPORT_DIR"/*.onnx &>/dev/null; then
    ...
    bash "$SCRIPT_DIR/export_piper.sh" --lang "$LANG"
    echo "[Stage 3] Done."
fi

echo ""
echo "================================================================"
echo " Pipeline complete for lang=$LANG"
